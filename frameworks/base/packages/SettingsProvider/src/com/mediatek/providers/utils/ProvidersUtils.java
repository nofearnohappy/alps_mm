...
public class ProvidersUtils {
    ...
    public void loadCustomSystemSettings(SQLiteStatement stmt) {
        ...
        // M: Add for SIM-Mode
        boolean isSupport = SystemProperties.getBoolean("ro.mtk_gemini_support", false);
        int defResId = isSupport ? R.integer.def_dual_sim_mode
                : R.integer.def_single_sim_mode;
        loadIntegerSetting(stmt, Settings.System.MSIM_MODE_SETTING, defResId);

        // M: Add for Voice-wake-up
        isSupport = SystemProperties.getBoolean("ro.mtk_voice_unlock_support", false);
        defResId = isSupport ? R.integer.def_voice_unlock_mode
                : R.integer.def_voice_wakeup_mode;
        loadIntegerSetting(stmt, Settings.System.VOICE_WAKEUP_MODE, defResId);

        // M: Add for Audio-profile.
    ...
    public void updateAudioProfileActiveKey(SQLiteStatement stmt, SQLiteDatabase db) {
        ...
        try {
            db.beginTransaction();
            stmt = db
                    .compileStatement("UPDATE system SET value = ? " + "WHERE name = ?;");
    ...
    public String getFractionValue(String name, int resId, int defBase) {
        String defaultValue = Float.toString(mRes.getFraction(resId, defBase, defBase));
