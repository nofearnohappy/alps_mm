...
public class ExternalStorageProvider extends DocumentsProvider {
    ...
    public void deleteDocument(String docId) throws FileNotFoundException {
      ...
      Log.d(TAG, "deleteDocument file to be deleted = "+file);
        ...
        Log.d(TAG, "deleteDocument from Media DB starts");

        file = getFileFromDocIdMediaHelper(docId, true);
        Log.d(TAG, "deleteDocument file to be deleted from MediaDB = " + file);
      if(file != null)
      {
        String path = file.getAbsolutePath();
            Log.d(TAG, "deleteDocument file to be deleted from MediaDB path = " + path);
        Uri uri = MediaStore.Files.getContentUri("external");
            Log.d(TAG, "deleteDocument file to be deleted from MediaDB uri = " + uri);
        String where = MediaStore.Files.FileColumns.DATA + "=?";
        String[] whereArgs = new String[] { path};
        if (getContext() != null) {
                Log.d(TAG, "deleteDocument before content resolver");
        ContentResolver cr = getContext().getContentResolver();
            try {
                try {
                        Log.d(TAG, "deleteDocument before delete");
                        int i = cr.delete(uri, where, whereArgs);
                        Log.d(TAG, "deleteDocument after delete in resolver i= "+ i);
                } catch (UnsupportedOperationException e) {
                        Log.e(TAG, "Error, database is closed!!!");
                  }
                }
                catch (SQLiteException e) {
                        Log.e(TAG, "Error, Probem with deleting from Media DB!!!" + e);
                }
                }
    }
      else
      {
        Log.d(TAG, "File reference is NULL");
      }
    }
