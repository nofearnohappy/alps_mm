...
public final class ActivityStackSupervisor implements DisplayListener {
    ...
    void moveHomeStack(boolean toFront, String reason, ActivityStack lastFocusedStack) {
        ...
        if ((toFront && (stacks.get(topNdx) != mHomeStack))
            ...
            ActivityStackListener stackListener = getFocusedStack().mStackListener;
            if (null != stackListener && toFront == true) {
                stackListener.dumpStack(getFocusedStack());
            }
            /// @}
        }
    ...
    boolean resumeHomeStackTask(int homeStackTaskType, ActivityRecord prev, String reason) {
        ...
        mHomeStack.moveHomeStackTaskToTop(homeStackTaskType);
        ActivityRecord r;
        /// M: ALPS02346515 Improve the speed of first boot up @{
        if (mService.mBooting) {
            if (DEBUG_TASKS) {
                Slog.v(TAG, "Find home activity by getRunningHomeActivityForUser");
            }
    ...
    final boolean realStartActivityLocked(ActivityRecord r,
        ...
        /// M: AMS log enhancement @{
        if (DEBUG_ALL) Slog.v(TAG, "ACT-Launching: " + r);
        /// @}
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (err == ActivityManager.START_SUCCESS) {
            Slog.i(TAG, "START u" + userId + " {" + intent.toShortString(true, true, true, false)
                    + "} from uid " + callingUid + " from pid " + callingPid
    ...
    boolean finishDisabledPackageActivitiesLocked(String packageName, Set<String> filterByClasses,
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ArrayList<ActivityStack> stacks = mActivityDisplays.valueAt(displayNdx).mStacks;
            /// M: ALPS02328473: IndexOutOfBound. Stack be removed in "for" loop @{
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                final ActivityStack stack = stacks.get(stackNdx);
    ...
    /// M: ALPS02346515 Improve the speed of first boot up @{
    ActivityRecord getRunningHomeActivityForUser(int userId) {
        final ArrayList<TaskRecord> tasks = mHomeStack.getAllTasks();
        for (int taskNdx = tasks.size() - 1; taskNdx >= 0; --taskNdx) {
            final TaskRecord task = tasks.get(taskNdx);
            if (task.isHomeTask()) {
    ...
    boolean reportResumedActivityLocked(ActivityRecord r) {
        ...
        mService.notifyActivityState(r.packageName, r.getFocusAppPid(),
                r.realActivity.getShortClassName(), IActivityStateNotifier.ActivityState.Resumed);
        /// @}
    ...
    void setLockTaskModeLocked(TaskRecord task, int lockTaskModeState, String reason,
        ...
        /// M: ALPS02326165, Can't set lock task mode to home task @{
        if (task.isHomeTask()) {
    ...
    class ActivityContainer extends android.app.IActivityContainer.Stub {
        ...
        ActivityContainer(int stackId) {
            synchronized (mService) {
                ...
                /// M: AMS log enhancement @{
                if (mService.isAspLogOn == true) {
                    mStack.mStackListener = new ActivityStackListener();
                }
                /// @}
