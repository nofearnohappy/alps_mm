...
public class ServiceWatcher implements ServiceConnection {
    ...
    public boolean start() {
        synchronized (mLock) {
            if (!bindBestPackageLocked(mServicePackageName, true)) {
                return false;
            }
        }
     ...
     */
    private boolean bindBestPackageLocked(String justCheckThisPackage, boolean doBind) {
    ...
    private final PackageMonitor mPackageMonitor = new PackageMonitor() {
        ...
        public void onPackageUpdateFinished(String packageName, int uid) {
            synchronized (mLock) {
                ...
                bindBestPackageLocked(null, true);
            }
        }
    ...
    public void switchUser() {
        synchronized (mLock) {
            if (!mIsMultiuser) {
                unbindLocked();
                bindBestPackageLocked(mServicePackageName, true);
            }
        }
    }

   /**
     * M: Check best package is installed or not.
     * Return true if package was found.
     */
    public boolean isPackageInstalled() {
        synchronized (mLock) {
            return bindBestPackageLocked(mServicePackageName, false);
        }
    }
    // MTK add end
}
