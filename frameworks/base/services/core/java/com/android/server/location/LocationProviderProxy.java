...
public class LocationProviderProxy implements LocationProviderInterface {
    ...
    // MTK: support delay binding network location provider service
    private ProviderProperties mProperties = new ProviderProperties(
    ...
    // MTK: support delay binding provider service
    public boolean mDelayBinding = false;
    ...
    /**
      * M:  Create providerProxy if the specified package is installed, but not to bind it.
      * Return true if package was found.
      */
    public static LocationProviderProxy createAndDelayBind(
            Context context, String name, String action,
            int overlaySwitchResId, int defaultServicePackageNameResId,
            int initialPackageNamesResId, Handler handler) {
        LocationProviderProxy proxy = new LocationProviderProxy(context, name, action,
                overlaySwitchResId, defaultServicePackageNameResId, initialPackageNamesResId,
                handler);
        if (proxy.isPackageInstalled()) {
            proxy.mDelayBinding = true;
            return proxy;
        } else {
            return null;
        }
    ...
    private boolean isPackageInstalled() {
        return mServiceWatcher.isPackageInstalled();
    }

    //MTK added to support IPO
    ...
    private void unbind() {
        // MTK support delay binding provider service
        if (mDelayBinding && mBinded == false) {
            Log.d(TAG, "Skip unbind because service not yet binded");
            return;
        }
    ...
    private ILocationProvider getService() {
        // M: check if we should bind service now.
        if (mDelayBinding && mBinded == false) {
            Log.d(TAG, "getService mDelayBinding=" + mDelayBinding + " mBinded=" + mBinded);
            mBinded = true;
            bind();
        }
        // MTK add end
    ...
    public void disable() {
        ...
        // MTK support delay binding provider service
        if (mDelayBinding && mBinded == false) {
            Log.d(TAG, "Skip diable because service not yet binded");
            return;
        }
        // MTK add end
        ILocationProvider service = getService();
