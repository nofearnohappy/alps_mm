...
public final class HotKnotActivityManager extends IHotKnotCallback.Stub
    ...
    // Application(s) that have HotKnot state. Usually one
    final List<HotKnotApplicationState> mApps;
    ...
    void requestHotKnotServiceCallback(boolean request) {
        try {
            mAdapter.mService.setHotKnotCallback(request ? this : null);
        } catch (RemoteException e) {
            Log.e(TAG, "[HotKnotActivityManager] requestHotKnotServiceCallback()", e);
        }
    }

    boolean isHotKnotRequired(HotKnotActivityState state) {
        return (state.hotKnotMessage != null || state.hotKnotMessageCallback != null
                                || state.uriCallback != null || state.uris != null);
    }

    boolean checkMomPermission() {
        if (MobileManagerUtils.isSupported()) {
            return MobileManagerUtils.checkPermission(
                    SubPermissions.ACCESS_HOTKNOT, Binder.getCallingUid());
        }
        return true;
    }
