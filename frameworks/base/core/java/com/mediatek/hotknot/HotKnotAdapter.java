...
public final class HotKnotAdapter {
     ...
     * <p>The system inspects the MIME type of the HotKnot message and sends this intent containing the MIME type in its type field.
     * This allows the Activity to register IntentFilters targeting a specific MIME type. Activities should register the
     ...
     * {@link HotKnotMessage} when another HotKnot capable device is within range. You would use this
     * option where your application wants to:</p>
    ...
    public interface CreateHotKnotMessageCallback {
         ...
         * <p>This callback is usually made on a binder thread (not the UI thread).
         *
         * <p>Called when this device is in range of another HotKnot capable device. It enables the applications to
         * create a HotKnot message only when it is required.
         *
     ...
     * <p>Implement this interface and pass it to {@link
     * HotKnotAdapter#setHotKnotBeamUrisCallback setHotKnotBeamUrisCallback()} in order to package one or more URIs
     * for sending when another device capable of HotKnot transfer is within range. You would use this option where your
     * application wants to:</p>
    ...
    public static HotKnotAdapter getDefaultAdapter(Context context) {
        ...
        synchronized (sLock) {
            if (sDefaultService == null) {
                ...
                // SEF Proxy
                FeatureProxyBase sef = new FeatureProxyBase() {
                };

                IBinder binder = sef.getService(HOTKNOT_SERVICE);
                if (binder == null)
                    return null;
            ...
            adapter.mService = sDefaultService;
            return adapter;
        }
    }
