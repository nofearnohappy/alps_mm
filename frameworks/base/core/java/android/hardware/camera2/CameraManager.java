...
public final class CameraManager {
    ...
    public void setTorchMode(@NonNull String cameraId, boolean enabled)
        ...
        /* M:close flashlight when shutdown.*/
        closeFlashlightWhenShutdown(cameraId, enabled);
    }
...
/*===========================Mediatek added, close flashlight when shutdown======================*/
    private static final String ACTION_SHUTDOWN_IPO = "android.intent.action.ACTION_SHUTDOWN_IPO";
    private ArrayList<String> mFlashdeviceIdList = new ArrayList<>();
    private Handler mMainThreadHandler = new Handler(Looper.getMainLooper());
    private void closeFlashlightWhenShutdown(String cameraId, boolean enabled) {
        synchronized (mFlashdeviceIdList) {
            Log.i(TAG, "[closeFlashlightWhenShutdown]cameraId=" + cameraId + ",enabled=" + enabled
                    + ",list size=" + mFlashdeviceIdList.size());
            if (enabled) {
                mFlashdeviceIdList.add(cameraId);
                if (mFlashdeviceIdList.size() > 1) {
                    return;
                }
                mMainThreadHandler.post(new Runnable() {
                    public void run() {
                        ...
                        Intent intent = mContext.registerReceiver(mShutdownReceiver, filter);
                    }
                });
            } else {
                mFlashdeviceIdList.remove(cameraId);
                if (mFlashdeviceIdList.isEmpty()) {
                    mMainThreadHandler.post(new Runnable() {
                        public void run() {
    ...
    private final BroadcastReceiver mShutdownReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            Log.i(TAG, "[onReceive]intent=" + intent);
                synchronized (mFlashdeviceIdList) {
                try {
                    for (String cameraId : mFlashdeviceIdList) {
                        CameraManagerGlobal.get().setTorchMode(cameraId, false);
                    }
                    mFlashdeviceIdList.clear();
                } catch (UnsupportedOperationException exception) {
                    Log.w(TAG, "Clear mFlashdeviceIdList fail!");
                } catch (CameraAccessException exception) {
                    Log.w(TAG, "setTorchMode false fail!");
                }
            }
        }
    };
