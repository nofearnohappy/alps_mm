                ...
                case MTK_CAMERA_MSG_EXT_DATA_OT: {
                    ...
                    synchronized (mObjectCallbackLock) {
                        if (mObjectListener != null) {
                            Log.d(TAG, "OT callback0:" + intBuf.get(0));
                              if (intBuf.get(0) == 1) {
            ...
            try {
                if (moms != null) {
                    if (moms.checkPermission(permission, uid) != PackageManager.PERMISSION_GRANTED) {
                        Log.d(TAG, "open: user denied permission " + permission + " for uid " + uid);
                        return false;
                    }
                }
            } catch (RemoteException e) {
                Log.e(TAG, "CheckPermission failed in open with RemoteException, continues open.", e);
                return true;
            }
        }
        return true;
