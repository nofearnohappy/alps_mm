...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher,
    ...
    public void onInitializeAccessibilityNodeInfoForItem(
        ...
        final ListAdapter adapter = getAdapter();
        if (position == INVALID_POSITION || adapter == null || adapter.getCount() <= position) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (getChildCount() > 0) {
            ...
            /// M: [ALPS01470401] sync position has bee saved in setSelection, do not overwrite it
            if (!mNeedSync || mLayoutMode != LAYOUT_SPECIFIC) {
                rememberSyncState();
            }
        }
    ...
    private class FlingRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (mDataChanged || mIsDetaching || mAdapter == null || mItemCount == 0) {
                if (IS_ENG_BUILD && sDbgDataChange) {
