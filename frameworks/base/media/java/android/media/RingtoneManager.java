...
public class RingtoneManager {
    ...
    /**
     * Validate ringtone uri when MT call.
     *
     * @hide
     */
     ...
     */
    private static String constructBooleanTrueWhereClause(
            List<String> columns, boolean includeDrm) {
...
*/
    ...
    private static Ringtone getRingtone(final Context context, Uri ringtoneUri, int streamType) {
        try {
            /// M: { @ ALPS01762459 fixed sdcard ringtone file cannot play issue.
            int ringtoneType = -1;
            if (ringtoneUri != null) {
                ringtoneType = getDefaultType(ringtoneUri);
            }

            if (ringtoneType != -1) {
                ringtoneUri = getActualDefaultRingtoneUri(context, ringtoneType);
                if (ringtoneUri != null
                  && validRingtoneUri(context, ringtoneUri, ringtoneType) == -1) {
                    Log.d(TAG, "cannot found ringtone uri, return the Default uri.");
                    ringtoneUri = getDefaultRingtoneUri(context, ringtoneType);
                }
            }
            else {
                if (!isRingtoneExist(context, ringtoneUri)) {
                    switch (streamType) {
                        case AudioManager.STREAM_ALARM:
                             ringtoneType = TYPE_ALARM;
                             break;
                        case AudioManager.STREAM_NOTIFICATION:
                             ringtoneType = TYPE_NOTIFICATION;
                             break;
                        default:
                             ringtoneType = TYPE_RINGTONE;
                             break;
                    }
                    Log.w(TAG, "ringtone resource doesn't exist, infer type: "+ringtoneType);
                    ringtoneUri = getDefaultRingtoneUri(context, ringtoneType);
                }
            }
            /// @}
