...
#include <ui/Rect.h>
    ...
    #define EVENT_LOGD(...) if(g_HWUI_DEBUG_DETAILED_EVENTS) eventMarkDEBUG(__VA_ARGS__)
#else
    #define EVENT_LOGD(...)
#endif
...
OpenGLRenderer::OpenGLRenderer(RenderState& renderState)
        ...
        , mSpotShadowAlpha(0) {
    RecordingManager::getInstance().registerRecorder(&mRecorder);
}

OpenGLRenderer::~OpenGLRenderer() {
    RecordingManager::getInstance().unregisterRecorder(&mRecorder);
...
void OpenGLRenderer::discardFramebuffer(float left, float top, float right, float bottom) {
    ...
    if (mCaches.extensions().hasDiscardFramebuffer() &&
        ...
        TIME_LOG("glDiscardFramebufferEXT", glDiscardFramebufferEXT(GL_FRAMEBUFFER, 1, attachments));
    }
}
...
bool OpenGLRenderer::finish() {
    ...
    if (getTargetFbo() == 0) {
        ...
        /// M: save programs used for the first frame.
        /// This information can help to speed up next app launch time.
        static bool saved = false;
        if (!saved) {
            mCaches.programCache.saveToDisk();
            saved = true;
        }
    }
...
bool OpenGLRenderer::updateLayer(Layer* layer, bool inFrame) {
    if (layer->deferredUpdateScheduled && layer->renderer
        ...
        if (CC_UNLIKELY(inFrame || Properties::drawDeferDisabled)) {
            ...
            layer->renderer->recorder().prepare(*layer->renderer, layer);
            layer->defer(*this);
        }
...
void OpenGLRenderer::flushLayers() {
    ...
    if (count > 0) {
        ...
        // Note: it is very important to update the layers in order
        for (int i = 0; i < count; i++) {
            sp<Layer> layer = mLayerUpdates.itemAt(i);
            layer->flush();
            layer->renderer->recorder().finishRecording(*layer->renderer);
        }
        ...
        if (!bounds.isEmpty()) {
            if (layer->isEmpty()) {
                ...
                TIME_LOG("glTexImage2D", glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, layer->getWidth(),
                    layer->getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr));

                TT_ADD(layer->getTextureId(), layer->getWidth(), layer->getHeight(),
                    GL_RGBA, GL_UNSIGNED_BYTE, "createLayer", "[OpenGLRenderer.cpp] createLayer +");
...
bool OpenGLRenderer::createFboLayer(Layer* layer, Rect& bounds, Rect& clip) {
    ...
    if (layer->isEmpty()) {
        ...
        layer->setEmpty(false);
        TT_ADD(layer->getTextureId(), layer->getWidth(), layer->getHeight(), GL_RGBA,
            GL_UNSIGNED_BYTE, "layer", "[OpenGLRenderer.cpp] allocateTexture +");
    }
    TT_UPDATE(layer->getTextureId(), false);
...
void OpenGLRenderer::composeLayer(const Snapshot& removed, const Snapshot& restored) {
    ...
    if (fboLayer) {
        DUMP_LAYER(layer->getWidth(), layer->getHeight(), layer->getFbo(),
            recorder().getFrameCount(), this, layer);
        endTiling();
...
void OpenGLRenderer::composeLayerRegion(Layer* layer, const Rect& rect) {
    ...
    /// M: Define layer content range to reject quads outside of the real layer content bounday.
    android::Rect layerRange = android::Rect(0, 0, layer->layer.getWidth(), layer->layer.getHeight());

    TextureVertex quadVertices[count * 4];
    TextureVertex* mesh = &quadVertices[0];
    for (size_t i = 0; i < count; i++) {
        /// M: [ALPS01977103] Reject quads outside of the real layer content boundary by intersecting with layerRange.
        android::Rect rr = rects[i];
        rr.intersect(layerRange, &rr);
        const android::Rect* r = &rr;
...
void OpenGLRenderer::debugClip() {
    ...
    if (g_HWUI_DEBUG_CLIP_REGIONS) {
        if (!currentSnapshot()->getClipRegion().isEmpty()) {
            SkPaint paint;
...
void OpenGLRenderer::drawRenderNode(RenderNode* renderNode, Rect& dirty, int32_t replayFlags) {
    ...
    if (renderNode && renderNode->isRenderable()) {
        ...
        recorder().prepare(*this);
        ATRACE_BEGIN_L1("defer drawing commands");
        ...
        recorder().finishRecording(*this);
        DUMP_DISPLAY_LIST(getViewportWidth(), getViewportHeight(), recorder().getFrameCount(), this);
    } else {
