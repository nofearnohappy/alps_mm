...
void Layer::bindTexture() const {
    if (texture.id) {
        LAYER_RENDERER_LOGD("Layer %p bindTexture: glBindTexture(0x%x, %d)", this, renderTarget, texture.id);
...
void Layer::generateTexture() {
    if (!texture.id) {
        ...
        LAYER_RENDERER_LOGD("Layer %p generateTexture: glGenTextures(1, %d)", this, texture.id);
    }
}

void Layer::deleteTexture() {
    if (texture.id) {
        LAYER_RENDERER_LOGD("Layer %p deleteTexture: glDeleteTextures(1, %d)", this, texture.id);
...
void Layer::clearTexture() {
    LAYER_RENDERER_LOGD("Layer %p clearTexture %d, renderTarget 0x%x", this, texture.id, renderTarget);
...
#if DEBUG_LAYERS
    ALOGD("  Allocate layer: %p %dx%d", this, getWidth(), getHeight());
#endif
    if (texture.id) {
...
void Layer::flush() {
    ...
    if (deferredList && renderer) {
        ...
        if (renderNode != NULL) {
            TT_UPDATE(texture.id, false, renderNode->getName());
        } else {
