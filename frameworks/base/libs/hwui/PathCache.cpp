...
PathCache::PathCache():
    ...
    if (property_get(PROPERTY_PATH_CACHE_SIZE, property, nullptr) > 0) {
        INIT_LOGD("  Setting %s cache size to %sMB", "path", property);
        mMaxSize = MB(atof(property));
    } else {
        INIT_LOGD("  Using default %s cache size of %.2fMB", "path", DEFAULT_PATH_CACHE_SIZE);
    }
...
void PathCache::generateTexture(SkBitmap& bitmap, Texture* texture) {
    ...
    TT_ADD(texture->id, texture->width, texture->height, GL_ALPHA,
        GL_UNSIGNED_BYTE, String8("path"), "[PathCache.cpp] generateTexture +");
...
PathTexture* PathCache::get(const SkPath* path, const SkPaint* paint) {
    ...
    PATH_LOGD("PathCache::get path = %X, cached texture = %X, entry = %u",
...
void PathCache::precache(const SkPath* path, const SkPaint* paint) {
    ...
    if (generate) {
        ...
        // be enforced.
        mCache.put(entry, texture);
        PATH_LOGD("PathCache::precache path = %X, cached texture = %X, entry = %u",
            path, texture, (unsigned int)entry.hash());
