    ...
    while (1) {
		...
		case ITEM_TOUCH_CALIBRATION:
			...
			if(1 == g_pass_case1 && 1 == g_pass_case2 && 1 ==g_pass_case3)
			{
				...
				{					
                   g_detected = 2;
                   LOGD(TAG "Set g_detected = true for ATA.\n");
				}
				break;
			}
...
int touch_entry(struct ftm_param *param, void *priv) {
    ...
    do {
        ...
        case ITEM_TOUCH_CHOOSER:
			...
			if (get_is_ata()) {
                LOGD(TAG "Set tpd->scene = ITEM_TOUCH_MAIN for ATA.\n");
				tpd->scene = ITEM_TOUCH_MAIN;
			}
