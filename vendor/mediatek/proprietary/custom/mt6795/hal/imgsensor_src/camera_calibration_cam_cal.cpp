...
UINT32 DoCamCal2AGain(INT32 CamcamFID, UINT32 start_addr, UINT32 BlockSize, UINT32* pGetSensorCalData)
{
    ...
    UINT8 tempGain, CalR, CalGr, CalGb, CalG, CalB, FacR, FacGr, FacGb, FacG, FacB;
...
UINT32 DoCamCalLayoutCheck(void)
{
    ...
    if ((gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE) && (CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0]) == 0) && (OV13853_CAM_CALDeviceName(&cBuf[0]) == 0))
    {
...
*******************************************************************************/
/*##################################################################################################*/
UINT32 S5K3M2MIPIRAW_CAM_CALGetCalData(UINT32* pGetSensorCalData)
{
    UCHAR cBuf[128] = "/dev/";
    UCHAR HeadID[5] = "NONE";
    UINT32 result = CAM_CAL_ERR_NO_DEVICE;
...
//    CAM_CAL_LOG_IF(dumpEnable,"CAM_CALGetCalData(0x%8x)----\n",(unsigned int)pGetSensorCalData);
    ...
    if(DoCamCalLayoutCheck()==CAM_CAL_ERR_NO_ERR)
    {
        ...
        if ((CAM_CALInit() != CAM_CAL_NONE) && (CAM_CALDeviceName(&cBuf[0]) == 0) && (OV13853_CAM_CALDeviceName(&cBuf[0]) == 0))
        {
...
UINT32 S5K3M2_2ND_MIPIRAW_CAM_CALGetCalData(UINT32* pGetSensorCalData)
{
    UCHAR cBuf[128] = "/dev/";
    UCHAR HeadID[5] = "NONE";
    UINT32 result = CAM_CAL_ERR_NO_DEVICE;
...
UINT32 OV13853MIPIRAWCAM_CALGetCalData(UINT32* pGetSensorCalData)
{
    UCHAR cBuf[128] = "/dev/";
    UCHAR HeadID[5] = "NONE";
    UINT32 result = CAM_CAL_ERR_NO_DEVICE;
